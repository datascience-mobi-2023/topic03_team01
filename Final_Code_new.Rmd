...
#further separation into Dataframes with only one Rep, to simplify coding the analysis for us 
ctrl1 = ctrl[seq(1, ncol(ctrl), by = 3)] #after looking at the dataframe, every third column belongs to one Rep, separation by 3 
...
zeros_ctrl = c(remove_ctrl1, remove_ctrl2, remove_ctrl3, remove_rnase1, remove_rnase2, remove_rnase3) #vector with the rows, which sum equals zero
...
for (i in seq(1, 73, by = 3)) {
 ...
 sums = c(sum(ctrl_zeros[, i]), sum(ctrl_zeros[, i + 1]), sum(ctrl_zeros[, i + 2]))
  ...
  # Determine the sum closest to the mean of the sums
...
replace_mean = function(x){ #create a function to apply on the data frame, this function iterates through values 2-24 and replaces every value with the mean value of three values 
  for (i in 2:24){
  x[i] = mean(x[(i-1):(i+1)]) 
  }
  return(x)
...
```{r}
#correlation  
#correlation between each replicate of RNase and control respectively, using pearson method
...
#applying ttest: first the loops checks for identical values in the fractions, if so a 1 is added to the p_values_ctrl vector
for (i in 1:4588){
...
#trying different numbers of clusters to choose the optimal 
k1 <- 1
kmeans_result1 = kmeans(ctrl_cluster, centers = k1)
#total within-cluster sum of squares
kmeans1_wss = kmeans_result1$tot.withinss #keep the total within-cluster sum of squares in a variable for later use on the elbow method 
kmeans1_wss
...
#create a vector with all total within-cluster sums of squares for the elbow plot
elbow = c(kmeans1_wss, kmeans2_wss, kmeans3_wss, kmeans4_wss, kmeans5_wss, kmeans6_wss, kmeans7_wss)

clusters = c(1,2,3,4,5,6,7)

#create an elbow plot to identify the right number of clusters
