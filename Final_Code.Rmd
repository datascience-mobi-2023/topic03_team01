...
#further separation into data frames with only one Rep, to simplify coding the analysis for us 
ctrl1 = ctrl[seq(1, ncol(ctrl), by = 3)] #after looking at the data frame, every third column belongs to one Rep, separation by 3 
...
#which function is used to store the indices of the row with zeros only, apply to iterate the code through the whole data frame, sum was used to identify rows with only zeros
...
zeros_ctrl = c(remove_ctrl1, remove_ctrl2, remove_ctrl3, remove_rnase1, remove_rnase2, remove_rnase3)  
ctrl_zeros = ctrl[-zeros_ctrl,] #new control data frame without the proteins, which have at least one replicate containing only zeros

zeros_rnase = c(remove_ctrl1, remove_ctrl2, remove_ctrl3, remove_rnase1, remove_rnase2, remove_rnase3)
rnase_zeros = rnase[-zeros_rnase,] #new RNase data frame without the proteins, which have at least one replicate containing only zeros, all replicates put back together
...
#Normalization
#Control data frame 
for (i in seq(1, 73, by = 3)) { #Iterate through the fractions (each fraction has 3 replicates)
  
 #Compute the sums of every replicate
 sums = c(sum(ctrl_zeros[, i]), sum(ctrl_zeros[, i + 1]), sum(ctrl_zeros[, i + 2]))
  
  #Determine sum closest to mean of sums 
  diffs = abs(sums - mean(sums))
  sum_least_difference = sums[which.min(diffs)]
  
  #Compute the normalization factors
  norm_factor = sum_least_difference / sums
...
#RNase data frame
#same steps were applied to the RNase data frame 
...
```{r}
#Scaling 
#separate the data frames once again, to create six data frames, each containing only one replicate 
#control replicates
...
#sum of columns divided by 100 to create a quotient to scale
...
#control
for (i in 1:nrow(ctrl_norm1)) { #for loop iterates through all rows 
  QX = sum(ctrl_norm1[i, ])/100 #creating quotient by dividing the sum of all fractions of one protein by 100 
  ctrl_norm1[i, ] = ctrl_norm1[i, ]/QX #dividing each fraction of one protein by this quotient
}
...
replace_mean = function(x){ #create a function to apply on the data frame, this function iterates through values 2-24 and replaces every value with the mean value of three values (i - 1, i, i + 1)
  for (i in 2:24){
  x[i] = mean(x[(i-1):(i+1)]) 
  }
  return(x)
...
ctrl_smooth1 = apply(ctrl_norm1, 1, replace_mean) #here function created above is applied, using apply, function iterates through each row, creating a new data frame ctrl_smooth1
...
ctrl_smooth1 = t(ctrl_smooth1) #data frames are transposed to create original formation of the data frames (not necessary, but better for our understanding)
...
#loop to compare every row with one another of these two data frames
...
#all proteins with values which are under 0.8 
...
#using a for loop to iterate through the data frame and compute a mean value out of the three replicates of each fraction
for (i in 1:25) {
  replicates_ctrl = seq(i, length.out = 3, by = 25) #choose according values of each fraction out of each rep 
...
ctrl_mean = do.call(cbind, ctrl_mean) # combine all mean values out of the list into a data frame 
...
#using a for loop to iterate through the data frame and compute a mean value out of the three replicates of each fraction
for (i in 1:25) {
  replicates_rnase = seq(i, length.out = 3, by = 25) #choose according values of each fraction out of each rep 
...
rnase_mean = do.call(cbind, rnase_mean) # combine all mean values out of the list into a data frame 
```

```{r}
#find global peaks
global_peak_finder = function(x) which(x == max(x)) #create a function which will find the max value
global_peak_ctrl_mean = apply(ctrl_mean, 1, global_peak_finder) #apply the global_peak_finder function on our mean data frames, vector will contain fractions of the global peaks 
global_peak_ctrl_mean #control if code worked as expected 
...
local_peak_finder = function(x) { #create a function 
  ...
  for (i in 3:(length(x) - 2)) {
    if (x[i] > x[i+1] & x[i] > x[i-1] & x[i] > x[i+2] & x[i] > x[i-2]) #iterates through the values and looks for values, which are bigger than the 2 values before and the 2 values after the value of interest itself 
      peaks = c(peaks, i)
  }
...
#a shift only occurred if the difference between the global peaks of control and RNase is larger than 1 or smaller than -1 (threshold)
#therefore no shift occurred if the difference between the global peaks is 0, 1 or -1 
#precipitated proteins, are proteins accumulated in the last fraction (25), in control and RNase 
...
precipitated = which(ctrl_gp == 25 & rnase_gp == 25) #identifying precipitated shifts 
```
...
global_maxima = cbind(ctrl_gp, rnase_gp) #create a singular data frame containing the global maxima of control and RNase
...
#here the amount of proteins in the fractions of the global peaks of control group is stored in empty vectors for each replicate 
...
for (i in 1:nrow(rnase_1)){ #iterates through rnase_1 
  fraction=ctrl_gp[i] #each index of the global peaks of each protein is stored in the vector fraction
  amountr1 = rnase_1[i, fraction] #the amount of protein for this fraction is saved as amountr1
...
#appling t-test: first the loops checks for identical values in the fractions, if so a 1 is added to the p_values_ctrl vector
for (i in 1:4588){
if (ap_r1[i] == ap_c1[i] | ap_r1[i] == ap_c2[i] | ap_r1[i] == ap_c3[i] | ap_r2[i] == ap_c1[i] | ap_r2[i] == ap_c2[i] | ap_r2[i] == ap_c3[i] | ap_r3[i] == ap_c1[i] | ap_r3[i] == ap_c2[i] | ap_r3[i] == ap_c3[i]){
  ...
  ttest = t.test(amount_rnase[i,], amount_ctrl[i,], alternative = "two.sided") #application of the t-test to the protein amount in the same fraction of different replicates 
...
length(which(p_values_ctrl==1)) #77 proteins with the same amount of protein in at least one replicate in control and RNase 

p_values_ctrl[4588] #0.696, used as a control protein to check the reliability of the p-values 
...
#rnase peaks
#the same steps were applied on the fractions of the RNase global peaks 

#here the amount of proteins in the fractions of the global peaks of rnase group is stored in empty vectors for each replicate 
#create a vector with the protein amount of RNase in the fraction of the rnase global peak 
ap_r1_rev = c()
...
#create a vector with the protein amount of ctrl in the fraction of the rnase global peak 
ap_c1_rev = c()
...
p_val_adj_rev[4588] #control proteins to check the reliability of the p-values
...
```{r}
#k-means 
#clustering the positions of the global peaks of the control and RNase group 
ctrl_cluster = cbind(ctrl_gp, rnase_gp)

#trying different numbers of clusters to choose the optimal amount
k1 <- 1
kmeans_result1 = kmeans(ctrl_cluster, centers = k1)
#Sum of within square distance
kmeans1_wss = kmeans_result1$tot.withinss #keep the within square distance in a variable for later use on the elbow method 
kmeans1_wss
...
plot(clusters, elbow, type = "b", pch = 20, main = "Elbow method", xlab = "Number of clusters k", ylab = "total Within Sum of Square", col = "blue") #looking at this plot we identified 4 being the optimal number of clusters 
...
#create a vector containing all protein names 
...
df_lm = data.frame(row.names =  protein_names)
...
#create two data frames one with 90% of the rows of df_lm and one with the remaining 10% of rows 
...
predict_df = df_lm[4130:4588, ] #create new data frame with the remaining 10% of the rows -> test how well our model can predict 
...
#create vector with significant proteins 
...
#protein names of the significant proteins due to the second criteria (t-test)
...
#false_pos = sig.p - tp
#false_pos #number of false positive proteins 
...
col = c("steelblue1", "steelblue1", "steelblue1", "light blue", "light blue", "light blue")
bar_names_norm = c("cnorm1", "cnorm2", "cnorm3", "rnorm1", "rnorm2", "rnorm3")
bar_names = c("ctrl1", "ctrl2", "ctrl3", "rnase1", "rnase2", "rnase3")
barplot(plot_sums_bsca, names.arg = bar_names, col = col, ylab = "sum protein amount AHNK_HUMAN", main = "sum of protein before scaling", axisnames = TRUE)
barplot(plot_sums_asca, names.arg = bar_names_norm, col = col, ylab = "sum protein amount AHNK_HUMAN",  main = "sum of protein after scaling",  axisnames = TRUE)
...
#visualize shifts 
#left shift 
plot(as.matrix(rnase_mean)[1, ], type = "l", col = "steelblue1", xlab = "fraction", ylab = "protein amount", main = "AHNK_HUMAN") 
lines(as.matrix(ctrl_mean)[1, ], type = "l", col = "palevioletred1")
legend("topright", legend = c("RNase", "Control"), col = c("steelblue1", "palevioletred1"), lwd = 2)

#right shift
plot(as.matrix(rnase_mean)[334, ], type = "l", col = "steelblue1", xlab = "fraction", ylab = "protein amount", main = "ANXA5_HUMAN")
lines(as.matrix(ctrl_mean)[334, ], type = "l", col = "palevioletred1")
legend("topright", legend = c("RNase", "Control"), col = c("steelblue1", "palevioletred1"), lwd = 2)

#no shift 
plot(as.matrix(rnase_mean)[4588, ], type = "l", col = "steelblue1", xlab = "fraction", ylab = "protein amount", main = "ZPR1_HUMAN")
lines(as.matrix(ctrl_mean)[4588, ], type = "l", col = "palevioletred1")
legend("topright", legend = c("RNase", "Control"), col = c("steelblue1", "palevioletred1"), lwd = 2)

#precipitated 
plot(as.matrix(rnase_mean)[1268, ], type = "l", col = "steelblue1", xlab = "fraction", ylab = "protein amount", main = "GYS1_HUMAN")
lines(as.matrix(ctrl_mean)[1268, ], type = "l", col = "palevioletred1")
legend("topleft", legend = c("RNase", "Control"), col = c("steelblue1", "palevioletred1"), lwd = 2)
